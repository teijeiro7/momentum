...
export default function FocusMode() {
  ...
  const getCurrentDuration = () => {
    ...
    return mode === 'work' ? STRATEGIES[strategy].work : STRATEGIES[strategy].break;
  };

  const progress = ((getCurrentDuration() - timeLeft) / getCurrentDuration()) * 100;
  const totalFocusTime = completedCycles * (strategy === 'custom' ? customWork : STRATEGIES[strategy].work / 60);
    ...
    <div className="max-w-4xl mx-auto animate-fade-in">
        ...
        {(Object.keys(STRATEGIES) as Strategy[]).map((key) => {
          ...
          return (
            <button
              ...
              style={{
                background: strategy === key ? 'var(--accent-glow)' : 'var(--bg-card)',
                border: strategy === key ? '2px solid var(--accent-neon)' : '2px solid var(--border)',
              }}
            >
              <div className="text-2xl mb-1">{strat.icon}</div>
              <div className="font-semibold mb-1" style={{ color: strategy === key ? 'var(--accent-neon)' : 'var(--text-primary)' }}>
                {strat.name}
              </div>
          ...
          <div className="relative" style={{ width: '240px', height: '240px' }}>
            ...
            >
              ...
              <circle
                ...
                cy="120"
                r="110"
                stroke={mode === 'work' ? 'var(--accent-neon)' : 'var(--accent-growth)'}
                strokeWidth="12"
                fill="none"
                ...
                style={{ 
                  filter: `drop-shadow(0 0 12px ${mode === 'work' ? 'var(--accent-neon)' : 'var(--accent-growth)'})`,
      ...
      <div className="grid grid-cols-2 gap-4 mb-8">
        ...
        >
          ...
          <p className="text-3xl font-bold" style={{ color: 'var(--accent-neon)' }}>
            {Math.floor(totalFocusTime / 60)}h {Math.floor(totalFocusTime % 60)}m
          </p>
          <p className="text-sm mt-1" style={{ color: 'var(--text-muted)' }}>
            Total Focus
          </p>
